...
const bech32_1 = require("../lib/bech32");
    ...
    try {
        ...
        if (addrType <= 0x07 || addrType === 0x0e || addrType === 0x0f) {
            ...
            if (addrType === 0x0e || addrType === 0x0f) {
                return (0, bech32_1.encodeBech32Address)(stakePrefix, bytes);
            }
            return (0, bech32_1.encodeBech32Address)(prefix, bytes);
        }
        ...
        if (headerByte === 0x82 || headerByte === 0x83) {
            return `byron_${(0, cbor_1.toHex)(bytes).substring(0, 64)}`;
        }
        // Fallback: hex representation
        return (0, cbor_1.toHex)(bytes);
    }
    catch (err) {
        logger_1.logger.debug(`Address decode error: ${err.message}`);
        return (0, cbor_1.toHex)(bytes);
    }
}
