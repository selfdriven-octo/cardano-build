...
/**
 * REST API server using built-in Node.js http module — no Express dependency.
 */

type RouteHandler = (params: Record<string, string>, query: Record<string, string>) => any;
  ...
  const routes: { pattern: RegExp; handler: RouteHandler }[] = [
    // Chain
    {
      ...
      handler: () => {
        ...
        if (!tip) return { height: 0, slot: 0, hash: null, timestamp: 0 };
    ...
    },
    {
      ...
      handler: (params) => {
        ...
        if (!block) return { _status: 404, error: 'Block not found' };
    ...
    // Transactions
    {
      ...
      handler: (params) => {
        ...
        if (!tx) return { _status: 404, error: 'Transaction not found' };
